...
void help( void ) 
{
    printf("usage: maslda [est] [data] [label] [answers] [settings] [alpha] [tau] [omega] [k] [random/seeded/model_path] [seed] [directory] [batch_size]\n");
    printf("       maslda [inf] [data] [label] [answers] [settings] [model] [directory]\n");
}
...
int main(int argc, char* argv[])
{
    ...
    if (strcmp(argv[1], "est") == 0)
    {
        ...
        int batch_size = atoi(argv[12]);

        slda model;
        model.init(alpha, tau, omega, num_topics, &c, &setting, init_seed, batch_size);
