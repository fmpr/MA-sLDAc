                ...
                const corpus * c, const settings * setting, double init_seed)
{
    ...
    pi_laplace_smoother = setting->PI_LAPLACE_SMOOTHER;
    lambda_laplace_smoother = setting->LAMBDA_LAPLACE_SMOOTHER;
    ...
    log_pi = new double ** [num_annot];
    for (int a = 0; a < num_annot; a ++)
    {
        log_pi[a] = new double * [num_classes];
        for (int c = 0; c < num_classes; c ++)
            log_pi[a][c] = new double [num_classes];
    }
    ...
    if(pi_estimation)
    {
        for (int a = 0; a < num_annot; a ++)
            for (int c = 0; c < num_classes; c ++)
                for(int l = 0; l < num_classes; l++)
                    if (c==l)
                        log_pi[a][c][l] = log(0.97);
                    else
                        log_pi[a][c][l] = log(0.01);
     }
    else
        {
        for(int n = 0; n < num_annot; n++)
            for(int c = 0; c < num_classes; c++)
                           {
                for (int l = 0; l < num_classes; l++)
                    log_pi[n][c][l] = log(true_pi[n][c][l] + pi_laplace_smoother);
            }
    }
}
...
void slda::zero_initialize_ss(suffstats * ss)
{
    ...
    if(pi_estimation)
        for (a = 0; a < num_annot; a ++)
            ...
            {
                ss->pi_total_stat[a][c] = 0.0;
                for(l = 0; l< num_classes; l ++)
                {
                    ss->pi_stat[a][c][l] = pi_laplace_smoother;
                    ss->pi_total_stat[a][c] += pi_laplace_smoother;
                }
            }
}
...
void slda::v_em(corpus * c, const char * start, const char * directory, const settings * setting)
{
    ...
    while (((converged < 0) || (converged > em_converged) || (i <= LDA_INIT_MAX+2)) && (i <= em_max_iter))
    {
        ...
        // m-step
        printf("**** m-step ****\n");
        rmse = mle(ss, ETA_UPDATE, lambda, c);
        ...
        for(int a = 0; a < num_annot; a++)
        {
            ...
            {
                for(int l = 0; l < num_classes; l++)
                    fprintf(pi_file, "%.4f\t", exp(log_pi[a][c_][l]));
                fprintf(pi_file, "\n");
            }
            fprintf(pi_file, "\n");
        }
...
double slda::mle(suffstats * ss, int eta_update, double ** lambda, corpus* c) // M-STEP
{
    int k, w, a, c_, l, d;
    ...
    if(pi_estimation && eta_update)
    {
        for(a = 0; a < num_annot; a++)
            for (c_ = 0; c_ < num_classes; c_++) 
                for (l = 0; l < num_classes; l++) 
                    log_pi[a][c_][l] = log(ss->pi_stat[a][c_][l]) - log(ss->pi_total_stat[a][c_]);
        ...
        if(fopen(pi_filename, "r")!=NULL)
        {
            for(a = 0; a < num_annot; a++)
                for (c_ = 0; c_ < num_classes; c_++) 
                    for (l = 0; l < num_classes; l++) 
                        sum_sqr += (exp(log_pi[a][c_][l]) - true_pi[a][c_][l]) * (exp(log_pi[a][c_][l]) - true_pi[a][c_][l]);
...
double slda::slda_compute_likelihood(document* doc, double** phi, double* var_gamma, double *lambda)
{
    ...
    for (l = 0; l < num_classes-1; l ++)
    {
        ...
        for(a = 0; a<num_annot; a++)
		if(doc->answers[a]!=-1)
            		likelihood += lambda[l] * log_pi[a][l][doc->answers[a]];  
...
double slda::slda_inference(document* doc, double* var_gamma, double** phi, double* lambda)
{
    ...
    while ((converged > var_converged) && ((var_iter < var_max_iter) || (var_max_iter == -1)))
    {
        ...
        for (l = 0; l < num_classes-1; l ++)
        {
            ...
            for(int a = 0; a < num_annot; a++)
        		if(doc->answers[a]!=-1)
                        	pp[l] += log_pi[a][l][doc->answers[a]];

            lambda[l] = exp(dot_prod + pp[l]) + lambda_laplace_smoother; 
            lambda_sum+=lambda[l]; 
        ...
        for(int a = 0; a < num_annot; a++)
		if(doc->answers[a]!=-1)
            		pp[num_classes-1] += log_pi[a][num_classes-1][doc->answers[a]];

        lambda[num_classes-1] = exp(pp[num_classes-1]) + lambda_laplace_smoother;
