...
void help( void ) 
{
    printf("usage: maslda [est] [data] [answers] [settings] [alpha] [k] [random/seeded/model_path] [seed] [directory]\n");
...
int main(int argc, char* argv[])
{
    ...
    if (strcmp(argv[1], "est") == 0)
    {
        ...
        double init_seed = atof(argv[8]);
        char * directory = argv[9];
        printf("models will be saved in %s\n", directory);
        make_directory(directory);

        slda model;
        model.init(alpha, num_topics, &c, &setting, init_seed);
