...
double softmax_f(const gsl_vector * x, void * opt_param)
{
    ...
    double ** lambda = gsl_param->lambda;
    double f, t, a1 = 0.0, a2 = 0.0, f_dot;
    int k, d, j, l, c, idx;
    double f_regularization = 0.0;

    // F: compute the L2-regularization penalty term; this prevents the weights eta freom getting too big...
    ...
    // F: compute the equation before eq. 9 from Wang2009
    f = 0.0; //log likelihood
    for (d = 0; d < ss->num_docs; d ++)
    {
        for(c = 0; c < model->num_classes-1; c++) // nao existe eta para a ultima classe
        {
            f_dot = 0.0;

            for (k = 0; k < model->num_topics; k ++)
                    f_dot += model->eta[c][k] * ss->z_bar[d].z_bar_m[k]; 
...
void softmax_df(const gsl_vector * x, void * opt_param, gsl_vector * df)
{
    ...
    for (d = 0; d < ss->num_docs; d ++)
    {
        for (k = 0; k < model->num_topics; k ++)
        {
            for(c = 0; c < model->num_classes-1; c++) // nao existe eta para a ultima classe
            {
                idx = c*model->num_topics + k;
                g = gsl_vector_get(df, idx) + ss->z_bar[d].z_bar_m[k]*lambda[d][c]; 
                gsl_vector_set(df, idx, g);
            }
        }
...
void softmax_fdf(const gsl_vector * x, void * opt_param, double * f, gsl_vector * df)
{
    ...
    gsl_vector * df_tmp = gsl_vector_alloc(df->size);
    double ** lambda = gsl_param->lambda;
    double t, a1 = 0.0, a2 = 0.0, g, f_dot;
    int k, d, j, l, c, idx;
    double f_regularization = 0.0;
    double* eta_aux = new double [model->num_topics];
    ...
    *f = 0.0; //log likelihood
    for (d = 0; d < ss->num_docs; d ++)
    {
        for(c = 0; c < model->num_classes-1; c++)
        {
            f_dot = 0.0;
            for (k = 0; k < model->num_topics; k ++)
            {
